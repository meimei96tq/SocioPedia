...
def detect_event_ajax(request):
    if request.is_ajax and request.method == "POST":
        ...
        print("len temporal_knowledge", len(temporal_knowledge))
        for event in tqdm(events):
            ...
            for knowledge in temporal_knowledge:
                ...
                # print(knowledge, knowledge.group_tweets_id, score, set(
                #             knowledge.tweet.text.lower().replace("#", '').replace(",", '').replace(".", '').split(
                #                 " ")))
                if (score != 0) and (
                        set(trend.lower().split(" ")).issubset(set(
                            knowledge.tweet.text.lower().replace("#", '').replace(",", '').replace(".", '').replace(
                                "\n", "").split(
                                " ")))):
