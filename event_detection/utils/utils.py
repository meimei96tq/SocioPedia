...
def plot_event_timeline(knowledgegraphs, events):
    ...
    for event in events:
        ...
        colors.append(color)
    fig = px.timeline(df, title='Temporal event distribution', x_start="Start time", x_end="End time",
                      y="Event", hover_name="Present Knowledge",
                      color="Event", color_discrete_sequence=colors)

    # fig.update_layout(xaxis_showgrid=True, yaxis_showgrid=True, font=dict(size=18), hoverlabel=dict(
    #     font_size=16,
    # )
    # )
    plot_div = plot(fig,
...
def plot_event_timeline_chart(knowledgegraphs, events):
    ...
    # print("label ", list_labels)
    # print("list_parents ", list_parents)
    # print("list_values ", list_values)
    # print("list_colors ",list_colors)
    fig.update_layout(xaxis_showgrid=True, yaxis_showgrid=True, font=dict(size=18), hoverlabel=dict(font_size=16))
    plot_div = plot(fig,
                    output_type='div',
                    include_plotlyjs=False,
                    show_link=False,
